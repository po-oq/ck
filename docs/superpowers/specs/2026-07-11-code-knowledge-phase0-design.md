...
第11章の完了条件をすべて満たし、Phase 0の検証結果をユーザーが承認した時点を、Phase 1への正式な移行判断点とする。自動テストの成功だけではPhase 1へ移行しない。3クライアントの手動検証結果、配布成果物、および技術的前提との差異を含む検証記録全体を承認対象とする。

Phase 0からPhase 1への移行手順は以下とする。

1. Phase 0の自動テストをすべて実行する。
2. 発行済みEXEの`self-check`を実行する。
3. Cursor、GitHub Copilot in VS Code、Claude Codeで`phase0_probe`を手動実行する。
4. `spikes/phase0/README.md`へ、実行環境、パッケージバージョン、生成物、全検証結果、判明した制約を記録する。
5. 要件定義書との乖離がある場合は、実装ノートへ記録して要件定義書を改訂する。乖離が解消・承認されるまで移行を停止する。
6. ユーザーがPhase 0の完了とPhase 1への移行を承認する。
7. Phase 1専用の設計書を作成して承認を得る。
8. 承認済みのPhase 1設計書を元に、Phase 1の実装計画を作成する。
9. Phase 1の本番プロジェクトを`src/`配下へ新規作成する。

Phase 1へ引き継ぐ成果物は以下とする。

- 検証済みの技術的前提
- 使用したNuGetパッケージとバージョン
- SQLite、FTS5、MCP、同時実行、発行に関する実測結果
- 自動テストの検証ケースと、再実行可能な手順
- 3クライアントの設定例と実機検証結果
- 配布に必要なファイルの一覧
- 検証中に判明した制約、差異、失敗事例

Phase 0のソースコードはPhase 1へ直接昇格させない。Phase 1では、承認済みのPhase 1設計に従って`src/`配下へ本番コードを新規実装する。Phase 0のコードは`spikes/phase0/`に残し、依存パッケージや実行環境を更新した際の回帰検証に使用する。検証ロジックをPhase 1で再利用する場合も、ソースファイルのコピーではなく、Phase 1の責務とインターフェースに合わせて改めて実装する。
