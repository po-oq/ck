...
発行方式はframework-dependentの単一ファイルである。対象マシンに.NET 10 Runtimeが必要だが、管理者権限やインストーラーは不要で、発行済みEXEを任意のローカルフォルダへ配置して実行できる。
...
クライアントへ登録する前に、発行済みEXEを同期対象外の安定したローカルフォルダへ配置する。以降の設定例では次の絶対パスを使用する。
...
- バージョン更新時はMCPクライアントを停止してから`CodeKnowledge.Mcp.exe`を上書きする。
- 配置フォルダを移動する場合は、MCPクライアントを停止し、`knowledge.db`、`knowledge.db-wal`、`knowledge.db-shm`をEXEと一緒に移動する。
